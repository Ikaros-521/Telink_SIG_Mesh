...
void set_pb_gatt_adv(u8 *p,u8 flags)
{
	...
	//LOG_MSG_LIB(TL_LOG_NODE_SDK,par, len,"set_pb_gatt_adv:\r\n",op);
	return;
}

/***************************add sg*************************************************/

void bat_set_adv_unprovision(rf_packet_adv_t * p) 
{
//	set_pb_gatt_adv(p->data,6);
	p->header.type = LL_TYPE_ADV_IND;
	memcpy(p->advA,tbl_mac,6);
	extern u8 scanrsp_advData[12];
	memcpy(p->data, scanrsp_advData, sizeof(scanrsp_advData));
	p->rf_len = 6 + sizeof(scanrsp_advData);
	p->dma_len = p->rf_len + 2;	
	LOG_MSG_LIB(TL_LOG_NODE_SDK,p->data, sizeof(scanrsp_advData),"bat_set_adv_unprovision:\r\n",0);
	return ;
} 
/****************************************************************************/
...
void set_adv_provision(rf_packet_adv_t * p) 
{
	...
	p->rf_len = 6 + 29;
	p->dma_len = p->rf_len + 2;	
	//LOG_MSG_LIB(TL_LOG_NODE_SDK,p->data, 29,"set_adv_provision:\r\n",0);
	return ;
} 
