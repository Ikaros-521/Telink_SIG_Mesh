    ...
    && ((!is_lpn_support_and_en))
        ...
        if(clock_time_exceed(gatt_adv_tick, gatt_adv_inv_us)||MI_SWITCH_LPN_EN || SPIRIT_PRIVATE_LPN_EN){
            ...
            if(0){  // just for compile
                ...
                #endif
			/**************************add sg************************/
			else if(provision_mag.gatt_mode == GATT_ADV_NORMAL_MODE)
			{
				bat_set_adv_unprovision(p);
...
int app_advertise_prepare_handler (rf_packet_adv_t * p)
{
    ...
    if(0 == mesh_tx_cmd_busy_cnt){
		...
		//	LOG_MSG_LIB(TL_LOG_NODE_SDK,0,0,"0 == mesh_tx_cmd_busy_cnt !p_buf=1:\r\n",0);
...
//	LOG_MSG_LIB(TL_LOG_NODE_SDK,0,0,"app_advertise_prepare_handler:\r\n",0);
	...
	//	LOG_MSG_LIB(TL_LOG_NODE_SDK,0,0,"mesh_tx_cmd_busy_cnt=%d:\r\n",mesh_tx_cmd_busy_cnt);
...
{
    LOG_MSG_LIB(TL_LOG_NODE_SDK,params, par_len,"rcv access layer,retransaction:%d,src:0x%02x dst:0x%02x;op:0x%04x, par:",cb_par->retransaction,cb_par->adr_src,cb_par->adr_dst, cb_par->op);
    ...
    if(!is_cfg_model(p_res->id, p_res->sig)){ // user should not handle config model op code
        ...
        if(IS_VENDOR_OP(cb_par->op)){
            if((cb_par->op >= VD_OP_RESERVE_FOR_TELINK_START) && (cb_par->op <= VD_OP_RESERVE_FOR_TELINK_END)){
				...
				LOG_MSG_LIB(TL_LOG_NODE_SDK,0,0 ,"cb_par->op >= VD_OP_RESERVE_FOR_TELINK_START cb_par->op:%02X,START:%02X-%02X", cb_par->op,VD_OP_RESERVE_FOR_TELINK_START,VD_OP_RESERVE_FOR_TELINK_END);
              //  return -1;  telink �������û���op ��ce �� DF
            }
        }
            #endif
        #endif
